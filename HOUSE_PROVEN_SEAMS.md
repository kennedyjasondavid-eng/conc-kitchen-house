...
| 5 | **Atomic multi-file publish** — N files pushed as N commits means a crash mid-publish ships an artifact set that never existed; a version sidecar that outruns its payload silently pins every client to stale data (the 2026-05-28 HUB incident) | `pushFilesToGitHubAtomic(token, files, msg)`: one commit via the Git Data API (blobs → tree → commit → ref PATCH); the ref update is the all-or-nothing flip, a crash before it is a clean no-op; retry once on 409/422; **no fallback to the split path by design** — fails loudly so the publish is retried whole | EXPO `index.html` `pushFilesToGitHubAtomic` (~:22502) · **DOOR `index.html` `ghPushFilesAtomic`** (ported 2026-08-17, silent-drift slice D1) | **DOOR's gap is CLOSED** — one publish was 10 per-file commits via `_ghPushFileNow`, so 9 of 10 CI runs per publish evaluated partial artifact sets; D1 ported the seam and DOOR now commits the whole set once. DOOR's port adds one thing worth carrying back: its per-file anti-clobber guards (`not_hydrated`/`empty_clobber`/`size_regression`) live in a **shared evaluator both write paths call**, so a batch path cannot silently lose a guard the single-file path has. `_ghPushFileNow` remains for genuine single-file writes (`sidePublish`) — one file is already one commit. MISE's feed publish is its own reviewed path |
...
```json
{
  "rows": [
    ...
    { "row": 5, "class": "atomic multi-file publish",
      "seam": [
        { "repo": "conc-kitchen-expo", "file": "index.html", "has": ["async function pushFilesToGitHubAtomic("] },
        { "repo": "conc-kitchen-door", "file": "index.html", "has": ["async function ghPushFilesAtomic(", "async function _ghEvaluateWriteGuards("] }
      ] },
...
- **CI-minutes (2026-08-17 aside — not a seam, recorded here because this section already maps the repos' CI + visibility):** the four private repos *bill* their gate runs (public house/DOOR are free). The Chromium-download cache + superseded-run cancel now cover **both gates that have a browser to cache — EXPO's `schedule-gate.yml` (PR #239) and PROOF's `proof-gates.yml` (PR #14)** (version-keyed cache, self-healing install, cancel scoped off `main`); the cache-cost line is closed on both. **MISE's `method-gate.yml` is a plain-Node suite (no Playwright — "nothing to install") that already cancels-in-progress, so it has no cache port** (an earlier draft of this note wrongly listed it as a candidate — audited + corrected 2026-08-17). A docs-only PR to a private repo still fires its gate, but MISE's is a fast plain-Node run, not a browser-download one.
- **Ride-alongs — the per-app pointer rollout:** a one-line registry pointer beside each app orientation doc's existing INSIGHTS pointer. **HUB landed 2026-08-16** (with the served-layout correction — `CONC_Production_Hub.html` deploys as `index.html`); **EXPO landed 2026-08-17** (PR #239, riding the CI-streamlining change); **DOOR landed 2026-08-17** (this sweep — DOOR is public, so the pointer PR is free); **PROOF landed 2026-08-17** (PR #14 — folded into its proof-gates cost-control PR, exactly the substantive PROOF PR the pointer was waiting for, so no dedicated gate run); **MISE landed 2026-08-17** (PR #84 — a cheap plain-Node gate run, since MISE has no browser download, so the deferral cost was small). **Rollout complete: the registry pointer is live in all five app orientation docs** (HUB · EXPO · DOOR · PROOF · MISE).
