...
- **CI-minutes (2026-08-17 aside — not a seam, recorded here because this section already maps the repos' CI + visibility):** those four private repos also *bill* their gate runs (public house/DOOR are free). **EXPO's `schedule-gate.yml` (PR #239) and PROOF's `proof-gates.yml` (PR #14) both cache the Chromium download + cancel superseded PR runs** (version-keyed cache, self-healing install, cancel scoped off `main`); MISE's `method-gate.yml` cancels-in-progress (cache state unaudited) — the one gate left to consider. A docs-only PR to a private repo still fires its full gate, which is why MISE's ride-along pointer below waits for a substantive PR there.
- **Ride-alongs — the per-app pointer rollout:** a one-line registry pointer beside each app orientation doc's existing INSIGHTS pointer. **HUB landed 2026-08-16** (with the served-layout correction — `CONC_Production_Hub.html` deploys as `index.html`); **EXPO landed 2026-08-17** (PR #239, riding the CI-streamlining change); **DOOR landed 2026-08-17** (this sweep — DOOR is public, so the pointer PR is free); **PROOF landed 2026-08-17** (PR #14 — folded into its proof-gates cost-control PR, exactly the substantive PROOF PR the pointer was waiting for, so no dedicated gate run). **Still pending: MISE `CLAUDE.md`** — the sole remaining; private, so its pointer waits for MISE's next substantive PR (or land deliberately for one gate run).
