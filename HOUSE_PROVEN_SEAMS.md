...
- **CI-minutes (2026-08-17 aside — not a seam, recorded here because this section already maps the repos' CI + visibility):** those four private repos also *bill* their gate runs (public house/DOOR are free). EXPO's `schedule-gate.yml` now caches the Chromium download + cancels superseded PR runs (PR #239); MISE's `method-gate.yml` already cancels-in-progress; **PROOF's `proof-gates.yml` has neither** — the standing cost candidate (same cache + concurrency port EXPO just took). A docs-only PR to a private repo still fires its full gate, which is why the ride-along pointers below wait for a substantive PR there.
- **Ride-alongs — the per-app pointer rollout:** a one-line registry pointer beside each app orientation doc's existing INSIGHTS pointer. **HUB landed 2026-08-16** (with the served-layout correction — `CONC_Production_Hub.html` deploys as `index.html`); **EXPO landed 2026-08-17** (PR #239, riding the CI-streamlining change); **DOOR landed 2026-08-17** (this sweep — DOOR is public, so the pointer PR is free). **Still pending: MISE `CLAUDE.md` · PROOF `INSIGHTS.md`** — both private, so a pointer-only PR would bill a gate run for a one-liner (see the CI-minutes aside above); fold each into that repo's next substantive PR, or land deliberately if closing the list is worth one run apiece.
