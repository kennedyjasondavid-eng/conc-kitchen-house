...
| **Shelf life / hold** | Scheduled chilled holds run **5 d (37 items), 6 d (22), 7 d (2), 8 d (4, e.g. Vegan Chilli)**. The deployed HACCP card/model now uses **≤7 d @ ≤4 °C + validated barrier** for ROP, but one older MISE method fragment still says **"use by 2 weeks @ ≤41 °F"** and needs cleanup. ⚠ **For Reduced-Oxygen Packaging (53 vac recipes), 14 days is too long unless a validated botulinum barrier and the current HACCP limit are documented.** Frozen holds (e.g. 11 d "Shape & Freeze" patties) are a separate regime; a 27 d "Shakshuka" + 9 ambiguous "?d" holds need classifying. |
...
**Two findings that reshaped the first sketch:** (1) the thermal controls mostly *exist* in MISE/CODEX, and the HACCP card/model is now deployed, but they are **not monitored or recorded** — that's the build; one legacy ordinary bag-label fragment still needs alignment to the ≤7-day ROP rule; (2) the cooling "blast-chill vs cooled overnight" question is **resolved** — the stew is blast-chilled to <4 °C within 2 h, then held refrigerated overnight and bagged the next morning (compliant); the remaining CCP-2 work is just to *record* the checkpoint temperatures. See annex §0 and §5.
